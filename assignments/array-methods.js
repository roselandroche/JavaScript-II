...
runners.forEach(function(runner) {
  ...
  fullName.push(runner.last_name);
});
...
// Problem 1
// Use .filter() to show who donated more than $100, they receive prize tier 1, and those who donated more than $200, they receive prize tier 2
let prizeLevelOne = runners.filter((runner) => {
  return runner.donation > 100 && runner.donation < 200;
});
console.log(prizeLevelOne);

let prizeLevelTwo = runners.filter((runner) => {
  return runner.donation > 200;
});;
console.log(prizeLevelTwo);

// Problem 2
// Change each id to be 3 digits, where the first digit indicates what iteration of the run it is (assuming that this is the first run)

// const map1 = array1.map(x => x * 2);
const padded = runners.map(runner => runner.id.toString().padStart(3, '10'));
console.log(padded);

// Problem 3
// Every odd numbered person gets a hot pink badge number, every even numbered person gets lime green
let hotPink = runners.filter(runner => runner.id % 2 === 0);
console.log(hotPink);

let limeGreen = runners.filter(runner => runner.id % 2 !== 0);
